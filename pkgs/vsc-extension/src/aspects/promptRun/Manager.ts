...
import { VscMessagePromptRun } from "@wrkspc/core/message";
...
export namespace PromptRunManager {
  ...
  export interface ExecuteStreamingHandlers {
    onChunk?: (
      chunk: PromptRunManager.ExecuteStreamChunk,
    ) => void | Promise<void>;
    onTextDelta?: (
      delta: string,
      chunk: PromptRunManager.ExecuteStreamChunk,
  ...
  export interface ExecuteSummary {
    ...
    steps: PromptRunManager.ExecuteStreamFinishEvent["steps"];
    finishReason?:
      | PromptRunManager.ExecuteStreamFinishEvent["finishReason"]
      | null;
    warnings?: PromptRunManager.ExecuteStreamFinishEvent["warnings"];
  }
...
export class PromptRunManager extends Manager {
  #messages: MessagesManager;
  #secrets: SecretsManager;

  constructor(parent: Manager, props: PromptRunManager.Props) {
    super(parent);
  ...
  trigger() {
    this.#messages.send({ type: "prompt-run-ext-trigger" });
  }
  ...
  async #handleExecutePrompt(payload: VscMessagePromptRun.WvExecutePayload) {
    ...
    const sendRunError = (error: string, resultId?: string) => {
      const payload: VscMessagePromptRun.ExtErrorPayload = {
        runId,
        promptId,
        timestamp: now(),
        error,
      ...
      this.#messages.send({ type: "prompt-run-ext-error", payload });
    };

    const sendRunUpdate = (resultId: string, delta: string) => {
      const message: VscMessagePromptRun.ExtUpdate = {
    ...
    try {
      ...
      const startedMessage: VscMessagePromptRun.ExtStart = {
      ...
      if (this.#cancelledRuns.has(runId)) {
        ...
        const completedMessage: VscMessagePromptRun.ExtComplete = {
        ...
        queue.add(async () => {
          ...
          try {
            ...
            if (result.success) {
              ...
              const resultCompletedMessage: VscMessagePromptRun.ExtResultComplete =
                {
              ...
              const failureMessage: VscMessagePromptRun.ExtResultComplete = {
                type: "prompt-run-ext-result-complete",
                payload: {
                  runId,
                  promptId,
      ...
      if (wasCancelled) {
        ...
        for (const job of jobs) {
          ...
          const cancelledMessage: VscMessagePromptRun.ExtResultComplete = {
            type: "prompt-run-ext-result-complete",
            payload: {
              runId,
              promptId,
  ...
  async #handleStopPromptRun(payload: VscMessagePromptRun.WvStop["payload"]) {
  ...
  async #execute(
    promptText: string,
    modelId?: string,
    options?: PromptRunManager.ExecuteOptions,
    extras?: PromptRunManager.ExecuteExtras,
    attachments?: Array<{ name: string; mime: string; base64: string }>,
    runtimeOptions?: PromptRunManager.ExecuteRuntimeOptions,
  ): Promise<
    | {
        success: true;
        request: LanguageModelRequestMetadata;
  ...
  > {
    ...
    try {
      ...
      let collectedText = "";
      let summary: PromptRunManager.ExecuteSummary | null = null;
      ...
      const stream = streamText({
        ...
        onError: onStreamingError
          ...
          : undefined,
        onFinish: async (event: PromptRunManager.ExecuteStreamFinishEvent) => {
      ...
      if (!summary) {
        summary = await (async (): Promise<PromptRunManager.ExecuteSummary> => {
          ...
          const fallbackSummary: PromptRunManager.ExecuteSummary = {
            text:
              typeof text === "string"
                ? text
                : collectedText.length > 0
