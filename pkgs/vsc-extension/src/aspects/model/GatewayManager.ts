...
import { Auth, AuthGateway } from "@wrkspc/auth";
import { ModelGateway, ModelVercel } from "@wrkspc/model";
import { createHash } from "node:crypto";
import {
  ModelResponse,
  modelResponseErrorData,
} from "node_modules/@wrkspc/model/src/response.js";
...
export class ModelsGatewayManager extends Manager {
  ...
  async #fetchAuthScoped(gateway: AuthGateway): Promise<ModelGateway.Response> {
    switch (gateway.type) {
      case "vercel":
  ...
  async #fetchAuthScopedVercel(): Promise<ModelGateway.ResponseVercel> {
    const apiKey = await this.#secrets.get("auth-vercel-gateway-key");
    if (!apiKey) return this.#fetchGlobal();

    const source: ModelGateway.SourceAuth = {
      type: "auth",
    ...
    let data: ModelGateway.ResponseVercelData;
    try {
      const vercelData = await gateway.getAvailableModels();
      data = {
        status: "ok",
        payload: vercelData as ModelVercel.Payload,
      };
    } catch (err) {
  ...
  async #fetchGlobal(): Promise<ModelGateway.ResponseVercel> {
    const source: ModelGateway.SourceGlobal = { type: "global" };
    ...
    if (response.ok) {
      try {
        const payload = (await response.json()) as ModelVercel.Payload;
        data = {
          status: "ok",
          payload,
        };
  ...
  #cache(
    sourceType: ModelGateway.SourceType,
