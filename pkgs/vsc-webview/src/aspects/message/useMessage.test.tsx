...
import { MessagesProvider, useMessage, useMessages, useOnce } from "./Context";
...
describe("message context", () => {
  ...
  it("delivers inbound messages to registered handlers", () => {
    ...
    const { unmount } = renderHook(
      () => {
        useMessage("settings-streaming-state", handler, [handler]);
      },
      { wrapper: Wrapper },
    );
  ...
  it("subscribes via useOn and cleans up on unmount", () => {
    ...
    function Subscriber() {
      useMessage("settings-streaming-state", handler, []);
      return null;
    }
