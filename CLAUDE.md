...
- `pnpm test:rust` - Run Rust tests (from workspace root)
- `pnpm test:rust:watch` - Watch Rust tests with watchexec (from workspace root)
- `pnpm test:rust:review` - Review insta snapshots (from workspace root)
...
- **Never accept insta snapshots yourself** - always prompt the developer to run `pnpm test:rust:review` or `pnpm run test:review` to accept snapshots
