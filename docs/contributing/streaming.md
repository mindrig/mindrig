...
- The extension also mirrors the aggregated payload through `prompt-run-execution-result` to satisfy the legacy non-streaming UI.
...
- The webview asks for persisted state via `settings-streaming-get`; the extension replies with `{ enabled: boolean }` from its `globalState` memento (`mindrig.workbench.streamingEnabled`).
- When the user toggles the checkbox, the webview sends `settings-streaming-set` with the new value so the extension can persist it.

### Cancellation

- The stop button posts `prompt-run-stop` with the current `runId`.
- The extension aborts the active `AbortController`, adds the `runId` to its cancellation set, and ultimately delivers a `prompt-run-error` with `"Prompt run cancelled."` alongside a `prompt-run-complete` payload where `success` is `false`.
...
- **Stop**: Button appears while a run is in-flight, disabled until `prompt-run-start` arrives. When clicked it sends `prompt-run-stop`, disables itself, and resets once the extension confirms cancellation or completion.
...
- Webview state updates are validated by `pkgs/vsc-webview/src/__tests__/streaming-assessment.test.tsx`, which dispatches synthetic `prompt-run-*` messages into the `Assessment` component and checks placeholders, streamed markdown, and error rendering.
...
- **Logging**: emit `prompt-run-start`, `prompt-run-complete`, and `prompt-run-error` traces with `runId`, `promptId`, and `resultId` (when available) so backend pipelines can correlate runs. Record cancellation sources (user stop vs. transport abort) to separate user actions from network issues.
- **Metrics**: track time-to-first-token (`prompt-run-update` timestamp minus `prompt-run-start`), overall run duration, and per-result success rates. These help surface regressions when providers change behaviour.
