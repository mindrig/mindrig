...
- [ ] [Capture package directories](#capture-package-directories): List all current packages under `./pkgs` and the legacy locations slated for relocation, saving the output to `plans/001-pkg-consolidation/inventory.md`.
- [ ] [Record manifest metadata](#record-manifest-metadata): Extract `name`, `version`, `private`/`publish`, and descriptions from each package's manifest into `inventory-data.json` and summarize the results in `inventory.md`.
- [ ] [Validate inventory artifacts](#validate-inventory-artifacts): Review the collected data for completeness and highlight any anomalies (missing descriptions, duplicates) in the inventory doc.
...
- Append the command outputs to `plans/001-pkg-consolidation/inventory.md` under dated headings.
...
- Write structured data to `plans/001-pkg-consolidation/inventory-data.json`.
